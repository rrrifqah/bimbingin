...
class _BookingDosenScreenState extends State<BookingDosenScreen>
  ...
  Future<void> _loadData({bool forceRefresh = false}) async {
    ...
    if (user == null) return;
    await context.read<BookingProvider>().fetchBookingByDosen(
      user.id!,
  ...
  Future<void> _approveBooking(BookingModel booking) async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            result ? 'Booking disetujui!' : 'Gagal menyetujui booking.',
          ),
  ...
  Widget build(BuildContext context) {
    ...
    final pendingList = allBookings
        .where((b) => b.status == 'pending')
        .toList();
    final riwayatList = allBookings
        .where((b) => b.status != 'pending')
        .toList();
            ...
            Row(
              children: [
                CircleAvatar(
                  ...
                  child: Text(
                    (b.namaMahasiswa ?? 'M').isNotEmpty
                        ? (b.namaMahasiswa ?? 'M')[0].toUpperCase()
                        : 'M',
                    style: TextStyle(
                      color: primaryColor,
                      fontWeight: FontWeight.bold,
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        b.namaMahasiswa ?? 'Mahasiswa ID ${b.mahasiswaId}',
                        style: const TextStyle(
                          fontWeight: FontWeight.bold,
                          fontSize: 14,
                          color: Color(0xFF2D3142),
